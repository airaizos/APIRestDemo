...
final class ViewModel {
    ...
    func updateNote(id: UUID, type: ReminderType, newDescription: String, newDate: Date, newComments: String, newAmount: String, newEvent:EventType, newBodyPart: PetSize) {
        ...
        if let index = notes.firstIndex(where:  {$0.id == id }) {
            ...
            let updatedNote = Note(id: id, type: type, description: newDescription, value: value, date: newDate, createdAt: notes[index].createdAt, comments: newComments, measure: newBodyPart, event: newEvent, updatedAt: Date.now)
